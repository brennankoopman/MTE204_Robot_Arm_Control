...
for i = 1:1:n % runs the jacobian method n times
  ...
  for a = 1:n
    ...
    %axis tight manual;
    %plot the ideal path
    plot3(X,Y,Z, 'g','linewidth',2,'DisplayName',sprintf('Ideal Linear Path'),'marker','o' );
    ...
    plot3(M([1:a],1),M([1:a],2),M([1:a],3), 'r','linewidth',3,'DisplayName',sprintf('path of arm') );
    %plot the current version of the arm
    plot3(Arm(1,:),Arm(2,:),Arm(3,:), 'bk', 'linewidth',3);
    lgd = legend('show', 'location', 'northwest');
    if a ==1
       print jacobiandiv.pdf -append
    end
    print jacobiandiv.pdf -append
    %pause(0.2);
    hold off;
